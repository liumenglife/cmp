create table ia_writeback_record (
    ...
    unique key uk_writeback_result_target (result_id, target_type, target_id),
    key idx_writeback_record_result (result_id, writeback_status),
    key idx_writeback_record_target (target_type, target_id, writeback_status)
);
...
create table ia_writeback_dead_letter (
    ...
    trace_id varchar(120),
    created_at timestamp not null,
    key idx_writeback_dead_letter_status (dead_letter_status, created_at)
);
...
create table contract_ai_summary_view (
    ...
    section_list_json text not null,
    citation_reference_json text not null,
...
create table contract_ai_risk_view (
    ...
    risk_item_list_json text not null,
    clause_gap_summary_json text not null,
    recommendation_summary_json text not null,
...
create table contract_ai_extraction_view (
    ...
    extracted_field_list_json text not null,
    clause_match_summary_json text not null,
