create table ia_ocr_job (
    ...
    unique key uk_ocr_idempotency (document_version_id, job_purpose, idempotency_key),
    key idx_ocr_job_document (document_asset_id, document_version_id, job_status)
);
...
create table ia_ocr_result_aggregate (
    ...
    page_summary_json text not null,
    ...
    created_at timestamp not null,
    updated_at timestamp not null,
    key idx_ocr_result_document (document_asset_id, document_version_id, result_status, default_consumable)
);
...
create table ia_ocr_table_region (
    ...
    cell_list_json text not null,
    header_candidate_list_json text not null,
...
create table ia_ocr_audit_event (
    ...
    key idx_ocr_audit_trace (trace_id, occurred_at),
    key idx_ocr_audit_object (document_asset_id, document_version_id, occurred_at)
);
