...
create index idx_ao_tool_definition_capability on ao_tool_definition(capability_code, risk_level);

create table ao_tool_definition_snapshot (
    snapshot_ref varchar(80) primary key,
    run_id varchar(80) not null,
    tool_name varchar(120) not null,
    schema_snapshot_json clob not null,
    sandbox_policy_ref varchar(120) not null,
    offload_policy_ref varchar(120) not null,
    created_at timestamp not null
);

create index idx_ao_tool_snapshot_run on ao_tool_definition_snapshot(run_id, tool_name, created_at);
...
create table ao_tool_grant (
    ...
    grant_scope_json clob not null,
    denied_reason varchar(512),
    created_at timestamp not null
);

create index idx_ao_tool_grant_run on ao_tool_grant(run_id, tool_name, grant_status);
...
create table ao_tool_result (
    ...
    resource_usage_json clob not null,
    created_at timestamp not null
);

create index idx_ao_tool_result_run on ao_tool_result(run_id, tool_invocation_id, result_status);
...
create index idx_ao_provider_usage_run on ao_provider_usage(run_id, provider_code, route_status);

create table ao_verification_report (
    verification_report_id varchar(80) primary key,
    run_id varchar(80) not null,
    verification_target varchar(256) not null,
    independent_context_sources_json clob not null,
    check_items_json clob not null,
    failure_evidence_json clob not null,
    performance_baseline_json clob not null,
    uncovered_risks_json clob not null,
    conclusion varchar(40) not null,
    regression_baseline_entry varchar(256) not null,
    created_at timestamp not null
);

create index idx_ao_verification_report_run on ao_verification_report(run_id, conclusion);
