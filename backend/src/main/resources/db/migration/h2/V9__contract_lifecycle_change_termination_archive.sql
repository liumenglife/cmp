create table cl_contract_change (
    ...
    result_version_no integer not null
);

create index idx_cl_change_contract on cl_contract_change(contract_id, change_status);
create index idx_cl_change_workflow on cl_contract_change(workflow_instance_id);
...
create index idx_cl_term_contract on cl_contract_termination(contract_id, termination_status);
create index idx_cl_term_workflow on cl_contract_termination(workflow_instance_id);
...
create unique index uk_cl_archive_contract_batch on cl_archive_record(contract_id, archive_batch_no);
create index idx_cl_archive_contract on cl_archive_record(contract_id, archive_status);
...
create index idx_cl_borrow_archive on cl_archive_borrow_record(archive_record_id, borrow_status);
create index idx_cl_borrow_contract on cl_archive_borrow_record(contract_id, borrow_status);
