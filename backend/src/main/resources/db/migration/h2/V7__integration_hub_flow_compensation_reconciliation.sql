...
create unique index uk_ih_outbound_attempt on ih_outbound_attempt(dispatch_id, attempt_no);
create index idx_ih_outbound_attempt_dispatch on ih_outbound_attempt(dispatch_id, attempted_at);
...
create table ih_recovery_ticket (
    ...
    ticket_round_no integer not null,
...
create unique index uk_ih_recovery_ticket on ih_recovery_ticket(resource_type, resource_id, failure_stage, ticket_round_no);
create index idx_ih_recovery_status on ih_recovery_ticket(recovery_status, manual_owner_id);
...
create index idx_ih_reconciliation_task on ih_reconciliation_task(system_name, task_status, window_end);
...
create index idx_ih_reconciliation_record_task on ih_reconciliation_record(reconciliation_task_id, record_result);

create index idx_ih_reconciliation_subject on ih_reconciliation_record(reconciliation_subject_key);
...
create unique index uk_ih_reconciliation_diff on ih_reconciliation_diff(diff_identity_key, baseline_fingerprint);
create index idx_ih_reconciliation_diff_record on ih_reconciliation_diff(reconciliation_record_id, current_state);
