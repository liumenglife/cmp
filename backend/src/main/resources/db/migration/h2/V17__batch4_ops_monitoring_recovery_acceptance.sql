create table ia_recovery_operation_log (
    ...
    result_payload_json clob not null,
    failure_reason varchar(512),
    created_at timestamp not null,
    completed_at timestamp
);

create index idx_recovery_operation_script on ia_recovery_operation_log(script_name, execution_status);
create index idx_recovery_operation_trace on ia_recovery_operation_log(trace_id, created_at);
...
create index idx_ops_maintenance_active on ia_ops_maintenance_window(subsystem, starts_at, ends_at);
...
create index idx_ops_alert_notification_rule on ia_ops_alert_notification(subsystem, metric_name, sent_at);
...
create index idx_ops_metric_snapshot_latest on ia_ops_metric_snapshot(subsystem, metric_group, metric_name, captured_at);
