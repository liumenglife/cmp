...
create index idx_translation_unit_term_language on ia_translation_unit(term_entry_id, language_code, status, version_no);

create table ia_terminology_profile (
    profile_code varchar(80) not null,
    profile_version int not null,
    domain_filter varchar(120),
    language_scope_json clob not null,
    included_term_keys_json clob not null,
    snapshot_payload_json clob not null,
...
create table ia_terminology_profile_term_snapshot (
    ...
    term_entry_id varchar(80) not null,
    translations_json clob not null
);

create index idx_terminology_snapshot_profile on ia_terminology_profile_term_snapshot(profile_code, profile_version);
...
create table ia_i18n_context (
    ...
    segment_language_payload_json clob not null,
    terminology_snapshot_json clob not null,
    downstream_degradation_json clob not null,
    created_at timestamp not null
);

create unique index uk_i18n_owner on ia_i18n_context(owner_type, owner_id);
...
create index idx_i18n_audit_target on ia_i18n_audit_event(target_type, target_id, occurred_at);
